...
//    */
//   return latency_table_add(line_addr, tag, cpu, pf, current_core_cycle & TIME_MASK);
// }
...
// uint16_t history_table_get_aux(uint32_t cpu, uint32_t latency, uint64_t tag, uint64_t act_addr, uint64_t ip[HISTORY_TABLE_WAY],
...
// uint16_t history_table_get(uint32_t cpu, uint32_t latency, uint64_t tag, uint64_t act_addr, uint64_t ip[HISTORY_TABLE_WAY], uint64_t addr[HISTORY_TABLE_WAY],
//                            uint64_t cycle)
// {
//   /*
...
//   uint16_t num_on_time = history_table_get_aux(cpu, latency, tag, act_addr, ip, addr, cycle);
...
//     find_and_update(cpu, latency, ip, current_core_cycle & TIME_MASK, line_addr);
...
//         //{
//         //     RR[RR_dx] = p_addr;
//         //     RR_cycle[RR_dx] = current_core_cycle;
//         //     RR_dx++;
//         //     if (RR_dx == SIZE_RR) RR_dx = 0;
//         // }
//         launched++;
//       }
//     }
...
// uint32_t CACHE:: prefetcher_cache_fill(uint64_t v_addr, uint64_t addr, uint32_t set, uint32_t way, uint8_t prefetch, uint64_t v_evicted_addr,
