                                        ...
                                        uint32_t metadata_in)
{
  ...
  if (trackers_l1[index].conf > 1 && trackers_l1[index].last_stride != 0 && flag == 1) { // CS IP
    meta_counter[1]++;
    total_count++;

    if (prefetch_degree[2] < 2)
      flag = 1;
    else
      flag = 0;

    for (int i = 0; i < prefetch_degree[2]; i++) {
...
// if(spec_nl == 1)
  ...
  if (CSPT_l1[signature].conf >= 0 && CSPT_l1[signature].stride != 0 && flag == 1) { // if conf>=0, continue looking for stride
    int pref_offset = 0, i = 0;                                                      // CPLX IP
    meta_counter[2]++;
    total_count++;

    for (i = 0; i < prefetch_degree[3] + CPLX_DIST; i++) {
      cspt_read_accesses++;
